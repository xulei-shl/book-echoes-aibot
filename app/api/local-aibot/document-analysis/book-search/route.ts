...
interface DocumentAnalysisBookSearchRequest {
    draftMarkdown: string;  // 文档分析生成的草稿
    userInput: string;      // 用户原始输入（通常是文档名称）
}
...
export async function POST(request: Request) {
    ...
    try {
        ...
        const { draftMarkdown, userInput } = body as DocumentAnalysisBookSearchRequest;
        ...
        logger.error('文档分析图书检索失败', { error });
        return NextResponse.json({
            message: '文档分析图书检索失败，请稍后重试',
            success: false
        }, { status: 500 });
    }
}