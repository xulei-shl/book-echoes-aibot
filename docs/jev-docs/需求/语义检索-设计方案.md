...
**两段式补召回**（模型推出的硬条件专用，**0 次额外 Jev**）：这类条件往往极其选择性（类目实测 `K92` 只占全馆 5/509 ≈ 1%；「近两年」升级成的 `pubYearFrom` 同样只剩一小撮），融合后 top-K 里常常一本都没有。因此只在**候选偏薄时**（`fused.length < RERANK_TOP_K`）用完整条件重跑一次本地召回 —— BM25 补算，查询向量已在 LRU、①①b 已缓存，不烧任何 Jev 请求。两轮结果按名次分合并（`mergeRecall`）。

> **覆盖全部四类条件**（`pubYearFrom` / `minRating` / `excludeFiction` / `callClasses`），不是只有类目：它们升级成硬过滤的路径完全一样，只让类目享受补救是不对称的。判据是 `plan.applied.some(e => e.source === 'model')` —— `resolveConstraints` 只在**确定性层没有同名条件**时才把条件记成 `source: 'model'`（有冲突时记 `rule-conflict` 并丢弃），所以它天然等价于「模型新加、且赶不上下推」。确定性条件不必重跑：它已下推，第一轮 lane 就只搜过允许集合，用同一份条件重跑会得到完全一样的结果。
...
sequenceDiagram
    ...
    Note over P,J: 为什么要拆：decode 严格，「任一题不合规整次作废」。类目题选项最多、最易不合规，<br/>混进①时它一失手就作废年份/评分档位与 facets —— 用一个筛选条件换掉整层理解不划算。<br/>且类目是**硬过滤**（能删结果、能让首屏为空），与只做微调的 facets 不是同一个权威等级。
    ...
    P->>C: systemOne(req)
    Note over P: 合并规则「**退回较粗的 l1**」<br/>l1 = __none__ → 不设条件（二级多半是照主题词猜的）<br/>l2 与 l1 不同源 → 取 l1（查 `resolveClcCode(l2).level1`，不切前缀）<br/>l2 与 l1 同源 → 取更具体的 l2<br/>两道题各自选项 ≤ 14 / ≤ 91，远低于 255 上限
...
| ①b `understandClass`<br/>1 次 / 3 问 | `query` · `boundary` | `call_class_l1` choice（13 大类 + `__none__`）<br/>`call_class_l2` choice（90 二级/三级 + `__none__`）<br/>`negation_present` noul（自含） | `mergeClassReading()` → `callClasses`<br/>`negation` | choice：option keys == criteria keys、choice == argmax<br/>未知 key / `__none__` → `undefined`（不设条件） |
...
| **权威等级** | ① 的输出（facets）被规定只能微调、不能删结果；类目是**硬过滤**（能删结果、能让首屏为空），不该由同一次解码同时承担 |
...
**类目条件的两道门**（与年份/评分不同，**不看 `constraint_strictness`**）：那道题问的是「年份、评分、虚构与否是不是硬条件」，与类目不是同一个判断；而 `call_class` 本身就在问「是不是在按类目筛」，再叠一道等于把同一个信号数两遍。保留的两道门是「调用方已显式传类目则让位（`rule-conflict`）」与「句中有否定则一律不用（`negated`）」。
...
| `understandClass`（①b）失败 | **不中断**：不设类目条件，**年份/评分档位与 facets 照常生效**，`degraded: ['understand-class']`。这是拆出独立请求换来的隔离 |
