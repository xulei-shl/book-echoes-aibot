...
export function eligibility(cands: RerankCandidate[]): RankedOutcome {
  ...
  if (!eligible.length || cands.batchHasMatch === false) return { abstained: true, items: [] };
...
| UI 数值 / 色点 / 折叠 | `{relevancePct}% 相关`（≥70 金 / ≥40 灰金 / 其余灰）+ 「相关度 < 30% 的 N 条」折叠区 | `ResultCard` / `ResultGallery` |
...
components/search/
  ...
  ResultGallery.tsx          结果画廊（网格 + shared element + 折叠区）
