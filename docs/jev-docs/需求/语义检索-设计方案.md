...
flowchart TD
    ...
    U["<b>① understandQuery</b> — 1 次 Jev 请求 / 10 题<br/>2× choice（intent / genre）<br/>5× score（宽召回·新鲜度·难度·年份·评分）<br/>3× noul（口碑·是否硬条件·有无否定）"]
...
> 注意阶段划分：意图、口味与硬条件档位是**同一个请求里的 10 道题**（合并成 1 个 RTT），不是多次请求；`wide` 仅 `deep` 模式。
>
> ⚠️ **题型与问题集已本轮重构**：程度类判断全部改为 `score` 档位，并新增 `genre_preference`（choice）、`year_floor` / `rating_floor`（score 档位）、`constraint_strictness` / `negation_present`（noul）。本小节正文中仍写着 `noul ×4`、`fits::* noul` 的代码片段与提示框是重构前的版本，**以实现（`lib/jev/questions.ts`）与上面那张字段映射表为准**。
...
sequenceDiagram
    ...
    Note over P,J: ── ① intent + facets + 硬条件档位 ── 1 次请求 / 10 个问题合并（全链路只付 1 个 RTT）
